...
static void TSB_Lanczos0(int Mc_AN, int spin, double *****Hks, double **invS);
...
double RecursionS_H2(int SCF_iter,
                    double *****Hks,
                    double ****OLP0,
                    double *****CDM,
                    double *****EDM,
                    double Eele0[2], double Eele1[2])
{
  ...
  static int numprocs,myid,ID,IDS,IDR,tag;
    ...
    ***********************************************/
    ...
    for (spin=0; spin<=SpinP_switch; spin++){
      ...
      ****************************************************/
      ...
      if (SCF_iter==1){
        TSB_Lanczos0( Mc_AN, spin, Hks, invS );
      }
      else{
...
void TSB_Lanczos0(int Mc_AN, int spin, double *****Hks, double **invS)
{
  static int firsttime=1;
  ...
  for (i=0; i<ct_on; i++){
    ...
    RU[0][i][i+1] = 1.0;
    Right_U0[spin][Mc_AN][0][i][i+1] = 1.0;
  }
	...
	jan = Spe_Total_CNO[WhatSpecies[jg]];
	  ...
	  for (m=0; m<ian; m++){
	    for (n=0; n<ct_on; n++){
	      ...
	      sum = 0.0;
	      for (k=0; k<jan; k++){
		sum += Hks[spin][ih][kl][m][k]*RCcnt[n][Bnum+k];
	      }
    ...
    ****************************************************/
	...
	for (n=0; n<ct_on; n++){
      ...
      ****************************************************/
	  ...
	  Right_U0[spin][Mc_AN][rl+1][n][i] = TmpCoes[n][i];   
	}
      } 
  ...
  ****************************************************/
  ...
  for (rl=0; rl<=RNUM[ct_AN]; rl++){
    for (i=1; i<=Msize[Mc_AN]; i++){
       printf("Mc_AN=%2d rl=%2d i=%2d LU=%10.7f\n",Mc_AN,rl,i,LU[spin][Mc_AN][rl][0][i]);
    }
  }
  ...
  for (Mc_AN=1; Mc_AN<=Matomnum; Mc_AN++){
    ...
    ****************************************************/
	...
	for (k=0; k<tno; k++){
	  for (rl=0; rl<=RNUM[Gc_AN]; rl++){
	    for (i=0; i<tno; i++){
	      tmp_Rho[spin][rl][k][i]  = Rho[spin][rl][k][i];
	      tmp_ERho[spin][rl][k][i] = ERho[spin][rl][k][i];
	    }
	  }
	}
      }
