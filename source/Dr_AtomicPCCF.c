/**********************************************************************
  Dr_AtomicPCCF.c:
     ...
     Dr_AtomicPCCF.c is a subroutine to calculate the derivative, with
     respect to R, of partial core charge density of one atom specified
     "Gensi".

  Log of Dr_AtomicPCCF.c:
...
double Dr_AtomicPCCF(int Gensi, double R)
{
  ...
  if (Spe_WhatAtom[Gensi]==0) return 0.0;
  ...
  if (Spe_VPS_RV[Gensi][Spe_Num_Mesh_VPS[Gensi]-1]<R){
    ...
    f1 = Spe_Atomic_PCC[Gensi][m-2];
    f2 = Spe_Atomic_PCC[Gensi][m-1];
    f3 = Spe_Atomic_PCC[Gensi][m];
    f4 = Spe_Atomic_PCC[Gensi][m+1];
